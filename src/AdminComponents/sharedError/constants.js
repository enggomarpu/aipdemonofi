export const REQUIRED_MESSAGE = "This field is required."
export const INVALID_EMAIL_MESSAGE = "Invalid Email."
export const First_Alphabet_Must_Be_Capital = "First Alphabet Must Be Capital"
export const Phone_Number = "Invalid Format"
export const Integers = "Invalid Format"
export const Comapany_Name = "Invalid Format"
export const Address = "Invalid Address"
export const CONFIRM_PASSWORD = "Password does not match"
export const URL_ERROR_MESSAGE = "Please enter valid URL"



export const HeadingEnum = {
    Dashboard : "Dashboard",
    AffiliatesList : "Affiliates List",
    AffiliatesListSubAccount : "Affiliates Account",
    AdminSubAccounts: "Admin Accounts",
    EventsCalender: "Calender Events ",
    PendingPosts : "Pending Posts",
    AffiliateProfile: "Affiliate Profile",
    Tags: "Tag Lists",
    Skills : "Skills Tag",
    Interest : "Interests Tag",
    TermsAndCondition : "Terms And Conditions",
    Myhub: "My HUB",
    AvailableCollaboration : "Collaboration" ,
    ColabReviewRequest:"Collaboration Request Review",
    Notifications:"Notifications"  

} 